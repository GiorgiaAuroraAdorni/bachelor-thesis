...
Nel primo approccio proposto, viene utilizzato il modello \emph{bag-of-words} per rappresentare i dati testuali.
...
Come funzione di loss è stato scelto l'errore quadratico medio, in inglese \emph{mean squared error (MSE)}, presentato nella sezione \ref{subsec:loss}.
...
Prendendo in considerazione le tre diverse architettura implementate, valutiamo l'efficacia dei nostri risultati.
...
\begin{table}[H]
	...
	\begin{tabular}{l@{\hspace{.5cm}}ccc}
		...
		\bottomrule 
	\end{tabular}
	\captionof{table}{Confronto dei risultati in termini di \emph{loss} ottenuti dalle tre diverse architetture}
